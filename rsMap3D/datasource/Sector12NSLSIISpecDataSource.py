...
# This seems changed at some point, at least for data at NSLS-II 4ID ones.
TIFF_FILE_MERGE_STR = "%s_%%04d/%s_%%04d_%%05d.tif"
# Some messed up SPEC macro started the point # at 116 instead of 0
#  to account for that, here we have:
MESSYSCANS = [27, 28, 34, 45, 59]

class Sector12NSLSIISpecDataSource(SpecXMLDrivenDataSource):
    '''
    ...
    def __init__(self, 
        ...
        :rtype: Sector12NSLSIISpecDataSource
        '''
        super(Sector12NSLSIISpecDataSource, self).__init__(projectDir, 
                                                     projectName, 
                                                     projectExtension,
                                                     instConfigFile, 
                                                     detConfigFile, 
        ...
        for scannr in scans:
            ...
            for ind in range(len(scan.data[list(scan.data.keys())[0]])):
                if imageToBeUsed[scannr][ind] and mask[ind]:    
                    # read tif image
                    # Now here is  something really messy for Hua's data:
                    #  to handle the filename sequence number not starting at 0
                    #  This is hard coded in, it does not worth the trouble to 
                    #  change the class to be more generic.
                    #-------------------
                    if scannr in MESSYSCANS:
                        POINT_NUM_OFFSET = 116
                    else:
                        POINT_NUM_OFFSET = 0
                    im = Image.open(self.imageFileTmp % (scannr, scannr, ind+POINT_NUM_OFFSET))
                    #-------------------
